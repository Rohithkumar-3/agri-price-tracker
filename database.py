"""
database.py — Python 3.10-3.14 + Streamlit Cloud compatible.
Uses cursor-based _query() instead of pd.read_sql_query() to avoid
the pandas/sqlite3 breakage on Python 3.14.
"""
...
# ── Safe query helper — replaces pd.read_sql_query ───────────────────────────
def _query(sql: str, params=None) -> pd.DataFrame:
    """Run SELECT, return DataFrame. Works on Python 3.14 where
    pd.read_sql_query(sql, sqlite3_conn) is broken."""
    c = _conn()
    try:
...
_SCHEMA = [
    ...
    """CREATE TABLE IF NOT EXISTS markets (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        market_name TEXT NOT NULL, state TEXT NOT NULL, district TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE(market_name, state))""",
    """CREATE TABLE IF NOT EXISTS price_records (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        commodity_id INTEGER NOT NULL, market_id INTEGER NOT NULL,
        price_date DATE NOT NULL, min_price REAL NOT NULL,
        max_price REAL NOT NULL, modal_price REAL NOT NULL,
        arrivals REAL DEFAULT 0, source TEXT DEFAULT 'Synthetic',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    """CREATE TABLE IF NOT EXISTS anomalies (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        price_record_id INTEGER, commodity_id INTEGER NOT NULL,
        market_id INTEGER NOT NULL, detected_date DATE NOT NULL,
        anomaly_type TEXT NOT NULL, severity TEXT NOT NULL,
        deviation_pct REAL, description TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
    """CREATE TABLE IF NOT EXISTS forecasts (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        commodity_id INTEGER NOT NULL, market_id INTEGER NOT NULL,
        forecast_date DATE NOT NULL, predicted_price REAL NOT NULL,
        lower_bound REAL, upper_bound REAL, model_name TEXT, confidence REAL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE(commodity_id, market_id, forecast_date))""",
    """CREATE TABLE IF NOT EXISTS ai_insights (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        commodity_id INTEGER, market_id INTEGER,
        insight_date DATE NOT NULL, insight_type TEXT NOT NULL,
        title TEXT NOT NULL, content TEXT NOT NULL,
        sentiment TEXT DEFAULT 'neutral',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
    """CREATE TABLE IF NOT EXISTS scheduler_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        job_name TEXT NOT NULL, status TEXT NOT NULL,
...
def read_commodities() -> pd.DataFrame:
    return _query("SELECT * FROM commodities ORDER BY name")
...
def create_market(market_name: str, state: str, district: str) -> int:
    ...
    try:
        ...
        c.commit()
        return c.execute("SELECT id FROM markets WHERE market_name=? AND state=?",
                         (market_name, state)).fetchone()["id"]
    finally:
        c.close()

def read_markets() -> pd.DataFrame:
    return _query("SELECT * FROM markets ORDER BY state, market_name")
...
def upsert_price_record(commodity_id, market_id, price_date,
    ...
    c = _conn()
    try:
        c.execute("""INSERT INTO price_records
...
def bulk_insert_prices(records: list):
    ...
    try:
        c.execute("BEGIN")
        c.executemany("""INSERT OR IGNORE INTO price_records
            (commodity_id,market_id,price_date,min_price,max_price,modal_price,arrivals,source)
            VALUES (?,?,?,?,?,?,?,?)""", clean)
        c.execute("COMMIT")
    except Exception:
...
def log_scheduler(job_name, status, records_processed=0, error_message=None):
    c = _conn()
    try:
        c.execute("INSERT INTO scheduler_log(job_name,status,records_processed,error_message)"
                  " VALUES (?,?,?,?)",
...
def read_scheduler_log(limit: int = 50) -> pd.DataFrame:
    return _query(f"SELECT * FROM scheduler_log ORDER BY executed_at DESC LIMIT {limit}")
...
def get_state_analytics() -> pd.DataFrame:
    return _query("""
        SELECT m.state, COUNT(DISTINCT m.id) AS num_markets,
...
def get_price_trend(commodity_id: int, market_id: int, days: int = 90) -> pd.DataFrame:
    return _query("""
        ...
        ORDER BY price_date
    """, [commodity_id, market_id, f"-{days} days"])
